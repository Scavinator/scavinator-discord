...
client.on(Events.ThreadDelete, async thread => {
  ...
  if (thread.parent.id === team_scav_hunt.discord_items_channel_id) {
    ...
    if (page) {
      ...
      await thread.parent.messages.edit(team_scav_hunt.discord_pages_message_id, {embeds: [await pages_embed(team_scav_hunt)]});
    }
  }
})
  ...
  if (interaction.commandName === "setupchannel") {
    ...
    if (interaction.commandName === "item") {
      ...
      if (!team_scav_hunt.discord_items_message_id) {
        const items_channel = await interaction.guild.channels.fetch(team_scav_hunt.discord_items_channel_id, {cache: true});
        const items_msg = await items_channel.send({embeds: [await items_embed()]})
      ...
      if (team_scav_hunt.discord_pages_channel_id) {
        ...
        if (!page_thread || !page_thread.discord_thread_id) {
          ...
          if (!team_scav_hunt.discord_pages_message_id) {
            const pages_channel = await interaction.guild.channels.fetch(team_scav_hunt.discord_pages_channel_id, {cache: true});
            const pages_msg = await pages_channel.send({embeds: [await pages_embed()]})
